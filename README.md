...
・ir_learning_remote.py（学習リモコン。SW1で受信モード。受信成功で青LED点灯。SW2で送信。データはjsonで保存されます。パルスデータの収集に使用できます）

・aircon_ir_node、pulses.py（Wi-Fi 経由で動作するエアコンリモコンです。詳細は aircon_ir_node フォルダ内の README を参照してください）

🎥 実機デモ動画（YouTube）

IR 送信と Wi‑Fi 通信を同時に動作させた様子を収録しています。

このリポジトリのカスタムファームウェアとサンプルコードが、  
実際にエアコンを制御している動作を確認できます。  
